...
public class EarthquakeCityMap extends PApplet {
	...
	//public static List<Marker> threatenedCityMarkers;
	...
	public void cityClick() {
		...
		for(Marker m : cityMarkers) {
			if(!m.isHidden() && m.isInside(map, mouseX, mouseY)) {
				...
				for(Marker mh : cityMarkers) {
					if(mh != lastClicked) {
						mh.setHidden(true);
					}
				}
				for(Marker mh : quakeMarkers) {
					EarthquakeMarker qm = (EarthquakeMarker)mh;
					if(qm.getDistanceTo(m.getLocation()) > qm.threatCircle()) {
						qm.setHidden(true);
					}
				}
				return;
	...
	public void earthquakeClick() {
		...
		for(Marker m : quakeMarkers) {
			EarthquakeMarker mm = (EarthquakeMarker)m;
			if(!m.isHidden() && m.isInside(map, mouseX, mouseY)) {
				lastClicked = mm;
				lastClicked.setClicked(true);
				for(Marker mh : quakeMarkers) {
				...
				for(Marker mh : cityMarkers) {
					if(mh.getDistanceTo(mm.getLocation()) > mm.threatCircle()) {
						mh.setHidden(true);
					}/* else {
	...
	private void unhideMarkers() {
		for(Marker marker : quakeMarkers) {
			EarthquakeMarker mm = (EarthquakeMarker)marker;
	...
	public List<Marker> getQuake() {
		return quakeMarkers;
	}
	
	public List<Marker> getCity() {
		return cityMarkers;
	}
